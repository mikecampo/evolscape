...
def main():
    ...
    if len(args) < 1:
        print '** ', os.pardir
        path_to_map = os.path.join('../../../../data/levels', "map1.tmx")
...
def demo_pygame(file_name):
    ...
    pygame.display.set_caption("tiledtmxloader - " + file_name + \
                                                        " - keys: arrows, 0-9")
    screen_width = max(1024, world_map.pixel_width)
    screen_height = max(768, world_map.pixel_height)
    ...
    sprite_layers[0].add_sprite(hero)
